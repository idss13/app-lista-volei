...
export function WhatsAppShare({
  ...
  function buildMessage() {
    ...
    for (const [cat, label] of categories) {
      const oficiais = players.filter((p) => p.categoria === cat && p.status === 'oficial')
      if (oficiais.length === 0) continue
      lines.push(`*${label}:*`)
      oficiais.forEach((p, i) => lines.push(`${i + 1}. ${p.nome}`))
      lines.push('')
    }
    ...
    if (espera.length > 0) {
      ...
      espera.forEach((p, i) => lines.push(`${i + 1}. ${p.nome} (${p.categoria})`))
      lines.push('')
    }

    lines.push(`Inscrições até: *${horarioLimite}*`)
    return lines.join('\n')
  }
  ...
  return (
    <button className="whatsapp-btn" onClick={handleShare} type="button">
